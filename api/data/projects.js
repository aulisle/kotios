export const a = [
  {
    id: 0,
    location: {
      lat: 60.182577,
      lng: 24.975972
    },
    projectType: 2,
    title: 'Sompasaarenlaituri 12',
    address: 'Sompasaarenlaituri 12, Kalasatama, Helsinki',
    tagline: 'Konsulttivetoinen osallistava asuntohanke',
    values: ['resources'],
    phases: {
      project: 2,
      plot: 2,
      group: 3
    },
    thumbnail:
      'https://www.fira.fi/wp-content/uploads/2019/06/SSL-julkisivu-merelta-800.jpg',
    images: [
      'https://www.fira.fi/wp-content/uploads/2019/06/SSL-julkisivu-merelta-800.jpg',
      'https://www.fira.fi/wp-content/uploads/2017/12/sompasaarenlaituri-12-alue.jpg',
      'https://www.saraco.fi/wp-content/uploads/2017/02/Kala-2_sis%C3%A4kuva1_4_4-1024x725.jpg'
    ],
    description: [
      'Helsingin Albatrossiksi nimetyn asunto-osakeyhtiön kaikki asunnot on nyt varattu. Sompasaarenlaituri 8:ssa sijaitsevan kohteen maanrakennustyöt on jo aloitettu ja keväällä 2019 tontille alkaa nousta upouusi asuinkerrostalo.',
      'Saracon ryhmärakennuttamishankkeena toteutettava talo tulee sijaitsemaan osoitteensa mukaisesti aivan rannan tuntumassa, ja useimmista asunnoista on näköala merelle. Kerrostaloon on suunniteltu yhteensä 47 laatuhuoneistoa, jotka on suunniteltu tulevien asukkaidensa tarpeiden mukaisiksi.'
    ],
    events: [],
    contacts: [
      {
        name: 'Rauni Kolehmainen',
        role: 'project-advisor',
        email: 'etunimi.sukunimi@saraco.fi',
        phone: '+358 40 703 1828',
        initials: 'RK'
      }
    ]
  },
  {
    id: 1,
    location: {
      lat: 60.158016,
      lng: 24.913122
    },
    projectType: 2,
    title: 'Malagankatu 2',
    address: 'Malagankatu 2, Helsinki',
    tagline: 'Firan konsulttivetoinen hanke',
    values: ['resources'],
    phases: {
      project: 3,
      plot: 2,
      group: 3
    },
    thumbnail:
      'https://www.fira.fi/wp-content/uploads/2017/12/jatkassari_kerrostalo_1360x900-1360x900.jpg',
    images: [
      'https://www.fira.fi/wp-content/uploads/2017/12/jatkassari_kerrostalo_1360x900-1360x900.jpg'
    ],
    events: [],
    contacts: [
      {
        name: 'V-P Rajala',
        role: 'project-advisor',
        email: 'email@email.com',
        phone: '040 1234567',
        initials: 'VR'
      }
    ]
  },
  {
    id: 2,
    location: {
      lat: 60.157767,
      lng: 24.912011
    },
    projectType: 2,
    title: 'Välimerenkatu 24',
    address: 'Välimerenkatu 24, Helsinki',
    tagline: 'Merellistä asumista kantakaupungissa',
    values: ['resources'],
    phases: {
      project: 4,
      plot: 2,
      group: 3
    },
    thumbnail:
      'https://www.saraco.fi/wp-content/uploads/2017/02/AH8A2142-1024x718.jpg',
    images: [
      'https://www.saraco.fi/wp-content/uploads/2017/02/AH8A2142-1024x718.jpg',
      'https://www.saraco.fi/wp-content/uploads/2017/02/AH8A1614-1024x682.jpg',
      'https://www.saraco.fi/wp-content/uploads/2017/02/AH8A1593-682x1024.jpg',
      'https://www.saraco.fi/wp-content/uploads/2017/02/AH8A1631-1024x682.jpg',
      'https://www.saraco.fi/wp-content/uploads/2017/02/AH8A2280-1024x682.jpg',
      'https://www.saraco.fi/wp-content/uploads/2017/02/AH8A1728-1024x682.jpg',
      'https://www.saraco.fi/wp-content/uploads/2017/02/AH8A1565-1024x682.jpg',
      'https://www.saraco.fi/wp-content/uploads/2017/02/AH8A1763-1024x682.jpg',
      'https://www.saraco.fi/wp-content/uploads/2017/02/AH8A2229-1024x682.jpg'
    ],
    description: [
      'Jätkäsaaressa, Välimerenkadun ja Länsisatamankadun kulmauksessa sijaitseva As Oy Helsingin Capri on valmistunut ja luovutettu asukkaille. Ryhmärakennuttamalla toteutettuun asuinkerrostaloon valmistui 44 asuinhuoneistoa, joiden asukkaat ovat jo saaneet huoneistot käyttöönsä.',
      'As Oy Helsingin Capri on toteutettu Helsingin kaupungin vuokratontille Hitas-ehdoin. Talon on suunnitellut Arkkitehtitoimisto Brunow&Maunula Oy.'
    ],
    events: [],
    contacts: [
      {
        name: 'Rauni Kolehmainen',
        role: 'project-advisor',
        email: 'etunimi.sukunimi@saraco.fi',
        phone: '+358 40 703 1828',
        initials: 'RK'
      }
    ]
  },
  {
    id: 3,
    location: {
      lat: 62.24147,
      lng: 25.72088
    },
    projectType: 0,
    title: 'Kankaan ryhmärakennushanke',
    address: 'Jyväskylä',
    values: ['rento'],
    tagline: 'Uniikki mahdollisuus aloittaa juuri sinunlaisesi hanke',
    phases: {
      project: 0,
      plot: 0,
      group: 0
    },
    thumbnail: '/torppala/torppala0.jpg',
    images: ['/torppala/torppala0.jpg'],
    events: [],
    contacts: [
      {
        name: 'V-P Rajala',
        role: 'project-advisor',
        email: 'email@email.com',
        phone: '040 1234567',
        initials: 'VR'
      }
    ]
  },
  {
    id: 4,
    location: {
      lat: 60.80043,
      lng: 21.40841
    },
    projectType: 4,
    title: 'Uudenkaupungin puukerrostalo',
    address: 'Uusikaupunki',
    tagline: 'Elämisen iloa',
    values: ['eco'],
    phases: {
      project: 1,
      plot: 1,
      group: 1
    },
    thumbnail: '/uusikaupunki/header.png',
    images: ['/uusikaupunki/header.png'],
    events: [],
    contacts: [
      {
        name: 'V-P Rajala',
        role: 'project-advisor',
        email: 'email@email.com',
        phone: '040 1234567',
        initials: 'VR'
      }
    ]
  },
  {
    id: 5,
    location: {
      lat: 60.426251,
      lng: 22.323354
    },
    address: 'Skanssi, Turku',
    projectType: 4,
    values: ['natural', '0', '1', '2', '3', '4', '5', '6', '7'],
    title: 'Skanssin Rento-hanke',
    tagline: 'TERVETULOA SKANSSIIN – edistyksellisen energiatehokkuuden kotiin',
    phases: {
      project: 0,
      plot: 0,
      group: 0
    },
    thumbnail: '/skanssi/thumbnail.jpg',
    images: [
      '/skanssi/header.jpg',
      '/skanssi/viitesuunnitelma-1.jpg',
      '/skanssi/viitesuunnitelma-2.jpg',
      '/skanssi/viitesuunnitelma-3.jpg'
    ],
    events: [
      {
        date: '31.10.2019',
        subject: 'Tontinluovutuskilpailu päättyy',
        address: ''
      }
    ],
    contacts: [
      {
        name: 'V-P Rajala',
        role: 'project-advisor',
        email: 'email@email.com',
        phone: '040 1234567',
        initials: 'VR'
      }
    ],
    description: [
      'Turun kaupunki asettaa yleisellä tarjouskilpailulla ammattirakentajille tarjottavaksi vapaarahoitteiseen asuntotuotantoon asuinkerrostalojen korttelialueen (AK-1) ja sitä palvelevan autopaikoitukselle varatun korttelialueen (LPA-1) osan Skanssin kaupunginosassa.',
      'Skanssissa pyritään kehittämään uudenlaisia energian tuotannon, käytön ja seurannan ratkaisuja. Alueella halutaan kehittää ja pilotoida rakennuskohtaisten ratkaisujen lisäksi myös alueellisia uusia energiaratkaisuja, kuten matalalämpöistä kaukolämpöverkkoa. Skanssin alueelle on laadittu energialiite, jossa sovitaan tavoista, joilla osapuolet edistävät uusiutuvan ja kierrätetyn energian käyttöä, veden- ja energiakulutuksen tehokkuutta sekä kaksisuuntaista energiakauppaa Skanssin alueella. Tontinsaajan tulee toteuttaa energialiitteen mukaiset pakolliset vaatimukset, minkä lisäksi tarjousten vertailussa lisäarvoa tuo sitoutuminen energialiitteen valinnaisten kohtien täyttämiseen.',
      'Kaupungin tavoitteena on korttelin nopea toteutuminen, laadukas suunnittelu ja rakentaminen sekä monipuolinen asuntojakauma.',
      'Tule toteuttamaan fiksumpi tapa omantyyliseen asumiseen!'
    ],
    history: [],
    neighbourhood: 'skanssi',
    housingAssociation: 'Asukasyhdistys perustetaan myöhemmin'
  },

  // Torppala
  {
    id: 6,
    location: {
      lat: 60.394134,
      lng: 22.291013
    },
    title: 'Torppalan aluesuunnittelu',
    projectType: 4,
    address: 'Torppala, Kaarina',
    tagline:
      'Torppala Kaarinan ja Turun rajalla, ekologinen kylä Itämeren rannalla, Ihmisen hyvä asua ja olla.',
    description: ['Torppala on Torppala'],
    phases: {
      project: 0,
      plot: 1,
      group: 0
    },
    values: ['eco', 'resources', 'natural', 'solar'],
    thumbnail: '/torppala/header.png',
    images: [
      '/torppala/header.png',
      '/torppala/torppala0.jpg',
      '/torppala/torppala1.jpg',
      '/torppala/torppala2.jpg'
    ],
    events: [
      {
        date: '13.8.2019',
        subject: 'Esittely',
        address: 'Kiinamyllynkatu 10 B2, 20520 TURKU'
      },
      {
        date: '16.8.2019',
        subject: 'I heart Pukki -kisakatsomo',
        address: 'Kiinamyllynkatu 10 B2, 20520 TURKU'
      },
      {
        date: '18.8.2019',
        subject: 'Esittely 2',
        address: 'Kiinamyllynkatu 10 B2, 20520 TURKU'
      }
    ],
    contacts: [
      {
        name: 'V-P Rajala',
        role: 'project-advisor',
        email: 'email@email.com',
        phone: '040 1234567',
        initials: 'VR'
      }
    ],
    numberOfApartments: 100,
    moveInDate: '2022',
    creationDate: '2019',
    socialLiving: 0,
    housingAssociation: 'Koti kaupungissa ry',
    neighbourhood: 'torppala'
  },

  // Casa urbana
  {
    id: 7,
    location: {
      lat: 60.156598,
      lng: 24.911837
    },
    title: 'Casa Urbana',
    projectType: 2,
    address: 'Järkäsaari, Helsinki',
    tagline:
      'Konsulttialoitteinen Hitas-kerrostalohanke Helsingin Jätkäsaaressa.',
    description: [
      'Casa Urbana on konsulttialoitteisena ryhmärakennuttamishankkeena toteutettava Hitas-kerrostalohanke Helsingin Jätkäsaaressa.',
      'Casa Urbana –hankkeen ryhmärakennuttamiskonsulttina toimii R-Rakennuttajat Oy. Yrityksellä on Suomen rakennusmarkkinoilla poikkeuksellista osaamista ryhmärakennuttamishankkeen johtamisessa.'
    ],
    phases: {
      project: 3,
      plot: 2,
      group: 3
    },
    values: ['5'],
    thumbnail: '/casa-urbana/header.jpg',
    images: ['/casa-urbana/header.jpg', '/casa-urbana/piha.jpg'],
    events: [],
    contacts: [
      {
        name: 'R-rakennuttajat Oy',
        role: 'project-advisor',
        email: ' ryhmarakennuttajat@gmail.com',
        phone: '+358 50 371 2972',
        initials: 'RR'
      }
    ]
  },

  // Saraco Kruunuvuorenranta
  {
    id: 8,
    location: {
      lat: 60.16772,
      lng: 24.936071
    },
    title: 'Romeo ja Julia',
    projectType: 2,
    address: 'Pojamankatu 6, Kruunuvuorenranta, Helsinki',
    tagline: 'Uniikkia asumista Kruunuvuorenrannassa',
    description: [
      'Romeo ja Julia ovat Saracon ryhmärakennuttamishankkeita, jotka tarjoavat ainutlaatuisen tilaisuuden ostaa uniikki koti uudenlaisella tavalla. Koon lisäksi voit valita asuntosi sijainnin talosta, ilmansuunnan, kerroksen sekä huoneluvun. Yhdessä arkkitehdin ja muiden asiantuntijoiden kanssa suunnittelet unelmakotisi jo ennen kuin rakennustyöt alkavat. Talojen yksiöt myydään ensisijaisesti isompien asuntojen laajennusosiksi, joten ne tulevat myyntiin viimeisenä.',
      'Romeo ja Julia koostuvat matalista rivitalo-osista, sekä kerrostaloista. Kerrostalojen asukkaat saavat ihailla rauhoittavia merimaisemia lasitetuilta parvekkeiltaan. Matalissa rivitaloissa asukkaat puolestaan nauttivat oman pihan yksityisyydestä.  Oletko intohimoinen kokkaaja? Teetkö töitä kotona? Otetaan se huomioon kotisi suunnittelussa ja rakennutetaan yhdessä koti juuri sinulle.',
      'Kohteet valmistuvat Kruunuvuorenrantaan vuoronperään: ensin rakennetaan Julia osoitteeseen Pojamankatu 6. Romeo nousee myöhemmin viereiselle tontille osoitteeseen Turumankatu 7.'
    ],
    phases: {
      project: 0,
      plot: 2,
      group: 2
    },
    values: ['5'],
    thumbnail: '/saraco/romeo-julia-thumbnail.jpg',
    images: ['/saraco/romeo-julia-thumbnail.jpg'],
    events: [],
    contacts: [
      {
        name: 'Rauni Kolehmainen',
        role: 'project-advisor',
        email: 'etunimi.sukunimi@saraco.fi',
        phone: '+358 40 703 1828',
        initials: 'RK'
      }
    ]
  },

  // Saraco Polariksenkatu
  {
    id: 9,
    location: {
      lat: 60.184262,
      lng: 24.98003
    },
    title: 'Polariksenkatu 9',
    projectType: 2,
    address: 'Pojamankatu 6, Kruunuvuorenranta, Helsinki',
    tagline: 'Uusi kaupunkikoti meren äärellä',
    description: [
      'Merenrantaan rajautuvaan Kipparin kortteliin nousee ryhmärakennuttamishankkeena nelikerroksinen pienkerrostalo, johon valmistuu yhteensä 18 korkealaatuista uudisasuntoa. Jokainen koti tulee vastaamaan asukkaidensa toiveita pohjaratkaisua ja materiaaleja myöten.',
      'Yhteisöllisyyttä arkeen tuovat talon monipuoliset yhteistilat sekä Kalasataman viihtyisät korttelipihat, joiden ansiosta kodin tunnelma laajenee oman asunnon ulkopuolelta naapurustoon saakka.'
    ],
    phases: {
      project: 4,
      plot: 2,
      group: 3
    },
    values: ['5'],
    thumbnail:
      'https://www.saraco.fi/wp-content/uploads/2017/01/2019-09-30-Saraco-Valmiit-Kohteet-18-1333x900.jpg',
    images: [
      'https://www.saraco.fi/wp-content/uploads/2017/01/2019-09-30-Saraco-Valmiit-Kohteet-18-1333x900.jpg',
      'https://www.saraco.fi/wp-content/uploads/2017/01/2019-09-30-Saraco-Valmiit-Kohteet-13-1-1024x682.jpg',
      'https://www.saraco.fi/wp-content/uploads/2017/01/2019-09-30-Saraco-Valmiit-Kohteet-14-682x1024.jpg',
      'https://www.saraco.fi/wp-content/uploads/2017/01/2019-09-30-Saraco-Valmiit-Kohteet-19-682x1024.jpg',
      'https://www.saraco.fi/wp-content/uploads/2017/01/2019-09-30-Saraco-Valmiit-Kohteet-11-682x1024.jpg',
      'https://www.saraco.fi/wp-content/uploads/2017/01/2019-09-30-Saraco-Valmiit-Kohteet-9-682x1024.jpg',
      'https://www.saraco.fi/wp-content/uploads/2017/01/keittio%CC%884-1024x734.jpg',
      'https://www.saraco.fi/wp-content/uploads/2017/01/kylpyhuone3-1024x683.jpg',
      'https://www.saraco.fi/wp-content/uploads/2017/01/kylpyhuone9-1-1024x683.jpg'
    ],
    events: [],
    contacts: [
      {
        name: 'Rauni Kolehmainen',
        role: 'project-advisor',
        email: 'etunimi.sukunimi@saraco.fi',
        phone: '+358 40 703 1828',
        initials: 'RK'
      }
    ]
  },

  // Saraco Juutinraumankatu 6
  {
    id: 10,
    location: {
      lat: 60.159067,
      lng: 24.919417
    },
    title: 'Juutinraumankatu 6',
    projectType: 2,
    address: 'Juutinraumankatu 6, Jätkäsaari, Helsinki',
    tagline: 'Saracon ensimmäinen ryhmärakennuttamiskohde',
    description: [
      'Kaksiportaisen asuinkerrostalon sekä kolmikerroksisen townhouse-osan kaikki asunnot ovat asukkaiden tarpeiden mukaisiksi suunniteltuja, yksilöllisiä koteja. Kerrostalohuoneistojen ja townhouse-osan muodostamaan kokonaisuuteen valmistui yhteensä 45 asuntoa. Kooltaan Juutinrauma 6:n kohteeseen rakennetut asunnot vaihtelivat yksiöstä 120 m²:n kokoisiin koteihin. Kohteen räätälöityihin ratkaisuihin lukeutuu muun muassa kattoterassille asennettu ulkoporeallas.',
      'Kohteen suunnittelusta vastasi Anttinen Oiva Arkkitehdit, projektiarkkitehteina toimivat Matti Huhtamies ja Sanna Meriläinen.'
    ],
    phases: {
      project: 4,
      plot: 2,
      group: 3
    },
    values: ['5'],
    thumbnail:
      'https://www.saraco.fi/wp-content/uploads/2017/01/2019-09-30-Saraco-Valmiit-Kohteet-1-1600x900.jpg',
    images: [
      'https://www.saraco.fi/wp-content/uploads/2017/01/2019-09-30-Saraco-Valmiit-Kohteet-1-1024x682.jpg',
      'https://www.saraco.fi/wp-content/uploads/2017/01/2019-09-30-Saraco-Valmiit-Kohteet-3-1024x682.jpg',
      'https://www.saraco.fi/wp-content/uploads/2017/01/2019-09-30-Saraco-Valmiit-Kohteet-5-1024x682.jpg',
      'https://www.saraco.fi/wp-content/uploads/2017/02/Huoneisto-1-1024x683.jpg',
      'https://www.saraco.fi/wp-content/uploads/2017/02/Huoneisto-11-1024x683.jpg',
      'https://www.saraco.fi/wp-content/uploads/2017/02/Huoneisto-9-1024x683.jpg'
    ],
    events: [],
    contacts: [
      {
        name: 'Rauni Kolehmainen',
        role: 'project-advisor',
        email: 'etunimi.sukunimi@saraco.fi',
        phone: '+358 40 703 1828',
        initials: 'RK'
      }
    ]
  },

  // Koti kaupungissa Sompasaari
  {
    id: 11,
    location: {
      lat: 60.181336,
      lng: 24.977307
    },
    title: 'Sompasaaren hanke',
    projectType: 0,
    address: 'Kaljaasi Auroorankuja, Kalasatama, Helsinki',
    tagline: 'Sompasaaren hanke – As. Oy Helsingin Sompasaaren Sumppi',
    description: [
      'Sompasaaren yhteisöllinen 39 asunnon kerrostalohanke on asukaslähtöinen ryhmärakennuttamishanke, jossa talon tulevat asukkaat vastaavat kohteen suunnitteluttamisesta ja rakennuttamisesta palkkaamiensa ammattilaisten avulla. Talo tulee osoitteeseen Kaljaasi Auroran kuja, Loviseholmin puiston viereen.',
      'Sisääntulokerroksessa sijaitsee kutsuva aula ja juhlatila, viidennen kerroksen katolla on puutarha, jonka yhteydessä on kesäkeittiö ja sauna. Kahdeksannen kerroksen saunatiloista avautuu näkymät pitkälle horisonttiin.',
      'Talo rakennetaan energiatehokkaaksi ja laadukkaaksi. Taloon tulee ekologinen maalämpö ja -kylmä sekä jäteveden lämmöntalteenottojärjestelmä. Huoneistoihin tulee miellyttävän ja tasaisen lämmön luova lattialämmitys, huoneistokohtainen ja säädettävä ilmanvaihto.',
      'Suunnitelmissa on perinteisten yhteistilojen lisäksi mm. 2 saunaosastoa, yhteinen monikäyttötila ja ruokasali yhteisruokailua ja muita tapahtumia varten, vierashuone sekä kuntosali. Talon katolle tullaan rakentamaan kattopuutarha, ja sen yhteyteen saunaosasto ja kesäkeittiö. Irtaimisto- ja pyörävarastot sekä verstaat rakennetaan kellarikerrokseen ja pysäköintihalli pihakannen alle.'
    ],
    phases: {
      project: 4,
      plot: 2,
      group: 3
    },
    values: ['5'],
    thumbnail:
      'http://www.hemistan.fi/wp-content/uploads/2016/06/sompasaari2.jpg',
    images: [
      'http://www.hemistan.fi/wp-content/uploads/2016/06/sompasaari2.jpg',
      'http://www.hemistan.fi/wp-content/uploads/2016/06/sompasaari_voimagraphics.jpg'
    ],
    events: [],
    contacts: [
      {
        name: 'Koti kaupungissa RY',
        role: 'association',
        email: 'sompasaari.rrhanke@gmail.com',
        initials: 'KK'
      }
    ]
  },

  // Koti kaupungissa Verkkosaari
  {
    id: 12,
    location: {
      lat: 60.191719,
      lng: 24.981389
    },
    title: 'Verkkosaaren hanke',
    projectType: 0,
    address: 'Kaljaasi Auroorankuja, Kalasatama, Helsinki',
    tagline: 'Verkkosaaren hanke – As Oy Kalasataman Messi',
    description: [
      'Yhteisiä aterioita naapureiden ja ystävien joukossa, nautiskelua kattosaunan lämmössä ja oleskelua viihtyisässä takkahuoneessa… Kalasatamaan rakentuu 40 kodin yhteisöllinen talo, jossa asutaan laadukkaasti ja nautitaan elämästä.',
      'As Oy Kalasataman Messistä tulee yhteisöllinen talo, johon tulee runsaasti kaikkien asukkaiden käytettävissä olevia yhteistiloja: pohjakerrokseen yhteiskeittiö ja olohuone, 7. kerroksen katolle kaksi saunaa ja takkahuone sekä kesäkeittiö ja kattoterassi. Neloskerrokseen tulee vierashuone, harrastehuone sekä kattoterassi laatikkoviljelyä varten.',
      'Talon pohjakerrokseen rakennetaan irtaimisto- ja pyörävarastot sekä verstastila. Pysäköinti tulee sisäpihalle pihakannen alle. Pysäköinnin lisäksi sisäpihalle suunnitellaan korttelin yhteistä viheraluetta.',
      'Verkkosaaren tontti on kahden pihakadun kulmassa. Tontille nousee L-kirjaimen muotoinen talo, jonka Kalasatamankadun puoleinen osa on seitsenkerroksinen ja Verkkosaarenkadun puoleinen osa nelikerroksinen. Tontilla on rakennusoikeutta yhteensä 3 600 kerrosalaneliömetriä, mikä tarkoittaa noin 2 500:aa huoneistoalaneliömetriä. Talo on osa seitsemän taloyhtiön korttelia, ja sen suunnittelee arkkitehtitoimisto Sarlin+Sopanen. Hankkeen rakennuttajakonsultiksi on valittu Rakennuttajatoimisto HTJ.'
    ],
    phases: {
      project: 4,
      plot: 2,
      group: 3
    },
    values: ['5'],
    thumbnail:
      'http://www.hemistan.fi/wp-content/uploads/2016/06/messi_kadunpuoli.png',
    images: [
      'http://www.hemistan.fi/wp-content/uploads/2016/06/messi_kadunpuoli.png',
      'http://www.hemistan.fi/wp-content/uploads/2016/06/messi_pihanpuoli.png',
      'http://www.hemistan.fi/wp-content/uploads/2016/06/messi_takkahuone.png'
    ],
    events: [],
    contacts: [
      {
        name: 'Koti kaupungissa RY',
        role: 'project-advisor',
        email: 'sompasaari.rrhanke@gmail.com',
        initials: 'KK'
      }
    ]
  }
]

export default a
